...
const AppContent: React.FC = () => {
  ...
  const NavItem = ({ view, icon, label }: { view: AppView, icon: React.ReactNode, label: string }) => {
    ...
    return (
      <button
        ...
        className={`w-full flex items-center ${isSidebarOpen ? 'gap-3 px-4' : 'justify-center px-0'} py-3 rounded-lg transition-colors ${currentView === view ? 'bg-wine-800 text-white shadow-lg' : 'text-wine-100 hover:bg-wine-800/50'}`}
  ...
  return (
    <div className={`flex h-screen bg-wine-50 dark:bg-slate-900 overflow-hidden font-sans transition-colors duration-300 ${darkMode ? 'dark' : ''} print:h-auto print:overflow-visible`}>
      ...
      {/* Sidebar */}
      <aside
        className={`${isSidebarOpen ? 'translate-x-0 w-64' : '-translate-x-full md:translate-x-0 w-64 md:w-20'} fixed md:relative z-30 h-full bg-wine-900 text-white transition-all duration-300 ease-in-out flex flex-col shadow-xl print:hidden`}
      >
        <button 
          onClick={() => updateHasSelectedModule(false)}
          className={`p-6 border-b border-wine-800 flex items-center ${isSidebarOpen ? 'gap-3 justify-start' : 'justify-center'} hover:bg-wine-800 transition-colors text-left w-full focus:outline-none`}
        >
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden print:overflow-visible print:h-auto">
        {/* Topbar */}
        <header className="h-16 bg-white dark:bg-slate-800 border-b border-wine-100 dark:border-slate-700 flex items-center justify-between px-6 shadow-sm z-20 transition-colors duration-200 print:hidden">
          <button onClick={() => setSidebarOpen(!isSidebarOpen)} className="text-wine-600 dark:text-wine-100 hover:text-wine-900 transition-colors focus:outline-none" title="Alternar Menu">
            <Menu size={24} />
          </button>
        ...
        {/* View Area */}
        <main className="flex-1 overflow-y-auto p-6 bg-wine-50 dark:bg-slate-900 transition-colors duration-200 print:overflow-visible print:h-auto print:p-0">
